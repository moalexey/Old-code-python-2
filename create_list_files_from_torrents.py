...
class TorrentParser(object):
    ...
    def tmp(self):
        return self.parsed_content.get('info')
...
if __name__ == '__main__':
    ...
    list_files_links_and_filenames_file = open(
        r'D:\script\review3d\finde_broken_links\list_files_links_and_filenames_file.txt', 'w')
    list_files_links_and_filenames_file_utf = open(
        r'D:\script\review3d\finde_broken_links\list_files_links_and_filenames_file_utf_8.txt', 'w')
    
    for number, link_and_file_number in enumerate(list_files_and_links):
        url, file_number = link_and_file_number.strip().split(' | ')
        torrent_file = r'D:\script\review3d\finde_broken_links\torrents\%s.torrent' % (
            file_number,)
        try:
            tp = TorrentParser(torrent_file)
            print tp.tmp()['name']
        except:
            print 'error' + file_number
        else:
            path = r'D:\script\review3d\download_books_from_torrent_tracker\%s' % (tp.tmp()['name'],)
            if os.path.exists(path):
                list_files_links_and_filenames_file.write(
                    '%s | %s | %s\n' % (
                        url, file_number, tp.tmp()['name']))
                print True
            else:
                list_files_links_and_filenames_file_utf.write(
                    '%s | %s | %s\n' % 
                        (url, file_number, tp.tmp()['name']))
                print False
        print number
...
'''
    ...
    torrent_file = r'D:\script\review3d\finde_broken_links\tmp_torrents\724553.torrent'
#sys.argv[1:]
    tp = TorrentParser(torrent_file)
    print tp.tmp()['name']#tp.get_tracker_url(), tp.get_creation_date(), tp.get_client_name(), tp.get_files_details()
...
data = open(r'D:\script\review3d\finde_broken_links\tmp_torrents\724553.torrent', 'rb').read()

torrent = decode(data)
print(torrent['info']['name'])
#print os.path.exists(r'D:\script\review3d\finde_broken_links\torrents\%s' % (torrent['info']['name'],))
...
list_files_links_and_filenames_file = open(
    r'D:\script\review3d\finde_broken_links\list_files_links_and_filenames.txt', 'w')

list_files_links_and_filenames = []

for link_and_file_number in list_files_and_links:
    url, file_number = link_and_file_number.strip().split(' | ')
    torrent_file = open(
        r'D:\script\review3d\finde_broken_links\torrents\%s.torrent' % (
            file_number,), 'r').read()
    try:
        file_name = torrent_file[torrent_file.index('name'
                ):torrent_file.index('12:piece')]
        ...
        list_files_links_and_filenames_file.write(
            '%s | %s | %s\n' % (
            url, file_number, file_name.split(':')[-1]))
    ...
    try:
        ...
        file_name = torrent_file[torrent_file.index('name.'
            )+7:torrent_file.index('12:piece')]
        file_name = file_name.decode('UTF-8')
        print('2')
        print(file_name)
    #file_name = torrent_file[:torrent_file.index('12:piece')]
    #file_name = torrent_file[torrent_file.index('name.'):]

    path = '%s' % (file_name,)
    if os.path.exists(path):
        # Создание архива
        z = ZipFile(r'D:\script\review3d\finde_broken_links\tmp_torrents\%s.zip' % (
            file_number,), 'w')
